...
-t/--flat tries to put all in the root directory of the ZIP file (long filenames!).
-l/--filelist selects by given text/TSV file. You might point with -c/--column to the paths,
e.g. "Full path" or "Vollpfad" (German) when working with TSV generated by X-Ways.
-b/--blacklist excludes files by regular expressions.
A text file with one regular expression per line is required.
-w/--whitelist does the opposit - only matching paths are zipped. If used, -b is ignored.
'''
