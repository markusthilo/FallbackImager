...
### Reporter
Using a JSON file generated by EwfImager this module can parse a textfile template and insert values by given keys. Syntax:
```
\jinsert{}{key}
\jinsert{r}{key}
\jinsert{rl}{key}
```
{r} deletes command if key is not present, {rl} deletes the line where key is not present.
